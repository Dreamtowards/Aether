...
void ClosestHitForward(inout RayIntersection rayIntersection : SV_RayPayload, AttributeData attributeData : SV_IntersectionAttributes)
{
	...
	{
		const float biasSign = sign(dot(fragInput.tangentToWorld[2], -viewWS)); // fragInput.tangentToWorld[2]

		RayDesc transmittedRay;
		transmittedRay.Origin = pointWSPos + biasSign * fragInput.tangentToWorld[2] * _RayTracingRayBias; //fragInput.tangentToWorld[2]
