    ...
    public class VoxelRegistry : ScriptableObject
    {
        ...
        public void GenerateAtlas(string texType, string filepath, int resolution = 512, bool isDRAM = false)
        {
            ...
            if (isDRAM)
            {
                ...
                {
                    if (TexLoadResize(v, "disp", tmpOriginal, resolution, resolution))
                        TexCopyChannel(tmpOriginal, tmpComposed, 0, 0);
                    
                    if (TexLoadResize(v, "rough", tmpOriginal, resolution, resolution))
                        TexCopyChannel(tmpOriginal, tmpComposed, 0, 1);
                    
                    if (TexLoadResize(v, "ao", tmpOriginal, resolution, resolution))
                        TexCopyChannel(tmpOriginal, tmpComposed, 0, 2);
                    
                    if (TexLoadResize(v, "metal", tmpOriginal, resolution, resolution))
            ...
            else
            {
                ...
                foreach (var v in Voxels)
                {
                    TexLoadResize(v, texType, atlas, resolution, resolution, idx * resolution);
                    idx++;
                }
            }
        ...
        private static bool TexLoadResize(VoxelProto v, string texType, Texture2D dstTex, int dstWidth, int dstHeight, int dstX = 0)
        {
            var subdir = v.IsFoliage ? "Foliage" : "Solid";
            string path = $"{Application.dataPath}/Art/Voxel/{subdir}/{v.Id}/{texType}.png";
            if (!File.Exists(path))
                return false;
